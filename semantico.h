...
void backtracking(int level)
{
    while (!state_stack.empty()) {
        ...
        if (symbols[at.first].empty() || at.second <= level)
            return;
        state_stack.pop();
        symbols[at.first].pop();
        if (symbols[at.first].empty())
            symbols.erase(at.first);
    }
}
...
bool checkNodes(Element* node, int level, valueReturning& ret)
{
    for (int i = 0; i < node->list.size(); i++) {
        if (node->list[i]->nodeType == BLOCK) {
            ...
            if (error)
                return true;
        } else if (checkSemantic(node->list[i], level, ret))
            return true;
    }
    return false;
}
    ...
    case FUNC_DECLARATION: {
        ...
        if (symbols.count(name)) {
            ...
            for (int i = 0; i < node->list[0]->list.size(); i++) {
                if (checkSemantic(node->list[0]->list[i], level + 1, ret))
                    return true;
            }
